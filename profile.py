...
for i in range( params.n ):
  ...
  if i == 0:
    ...
    node.addService(pg.Execute(shell="sh", command="sudo su BW840606 -c '/local/repository/slurm_setup.sh'"))
    node.addService(pg.Execute(shell="sh", command="sudo su BW840606 -c '/local/repository/MungeKey_Create.sh'"))
    ...
    #node.addService(pg.Execute(shell="sh", command="sudo su BW840606 -c 'cp /local/repository/source/* /users/BW840606'"))
    node.addService(pg.Execute(shell="sh", command="sudo su BW840606 -c '/local/repository/slurm_setup.sh'"))
    #node.addService(pg.Execute(shell="sh", command="sudo su BW840606 -c '/local/repository/MungeKey_Setup.sh'"))
    node.cores = 2
    node.ram = 4096
